...
export async function pier_handle(pier_ctx) {
  ...
  if (!pier_env.BOT_KV) {
    await pier_sendCombinedReply(pier_event.replyToken, pier_extraMessages, [{ type: 'text', text: 'KV storage is not configured.' }], pier_chatId, pier_env);
    return;
  }
  ...
  await pier_sendCombinedReply(pier_event.replyToken, pier_extraMessages, [{ type: 'text', text: pier_buildLeaderboardText(pier_members, pier_limit) }], pier_chatId, pier_env);
}
